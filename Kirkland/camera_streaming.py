...
def main():
    ...
    front_camera = cs.startAutomaticCapture(name=robotmap.Cameras.front_name, path=robotmap.Cameras.front_dev_address)
    
    # these two cameras are switched between, so no need to do auto capturing
    cargo_camera = UsbCamera(name=robotmap.Cameras.cargo_name, path=robotmap.Cameras.cargo_dev_address)
    hatch_camera = UsbCamera(name=robotmap.Cameras.hatch_name, path=robotmap.Cameras.hatch_dev_address)
