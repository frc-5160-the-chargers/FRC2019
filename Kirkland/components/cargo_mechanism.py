...
class CargoMechanism:
    ...
    def execute(self):
        ...
        if (self.outer_cargo_limit_switch.get() and power > 0.05): # inner switch is not pressed
            self.cargo_mechanism_motor.set(power)
        elif (self.inner_cargo_limit_switch.get() and power < 0.05): # outer switch pressed
            self.cargo_mechanism_motor.set(power)
        else:
